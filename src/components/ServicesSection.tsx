...
const services = [{
  ...
  features: ["Full rim, tire & exhaust cleaning", "Pre-wash foam bath & contact wash", "Claybar treatment & iron decontamination", "Machine waxing & paint protection"],
  price: "From $120",
  popular: false,
  ...
  process: ["Initial inspection and photography", "Thorough wheel and tire cleaning with specialized brushes", "Pre-wash foam application to loosen dirt and grime", "Two-bucket contact wash method with premium microfiber mitts", "Clay bar treatment to remove embedded contaminants", "Iron decontamination to eliminate brake dust and fallout", "Machine polishing to restore paint clarity and depth", "Premium wax application for long-lasting protection", "Final quality inspection and detailing"],
  duration: "3-4 hours",
  ...
  features: ["Complete vacuuming & steam cleaning", "Fabric & leather conditioning", "Dashboard & trim restoration", "Odor elimination treatment"],
  price: "From $150",
  popular: false,
  ...
  features: ["Full exterior & interior detailing", "Paint correction & protection", "Premium ceramic coating", "Exclusive Balkan craftsmanship"],
  price: "From $250",
  popular: true,
  ...
  process: ["Comprehensive vehicle inspection and consultation", "Complete exterior wash and decontamination", "Paint correction using multiple polishing stages", "Premium ceramic coating application for ultimate protection", "Full interior deep cleaning and conditioning", "Engine bay detailing and protection", "Headlight restoration and protection", "Tire and wheel ceramic coating", "Final quality assurance and 48-hour curing period"],
  duration: "6-8 hours (2-day service)",
  warranty: "90-day comprehensive warranty"
}];
export const ServicesSection = () => {
  const [selectedService, setSelectedService] = useState<typeof services[0] | null>(null);
  return <section id="services" className="bg-gradient-hero py-[66px]">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {services.map((service, index) => <Card key={index} className={`relative bg-card/50 backdrop-blur-sm border-border/20 shadow-card hover:shadow-luxury transition-all duration-300 hover:-translate-y-2 flex flex-col ${service.popular ? 'ring-2 ring-primary' : ''}`}>
              {service.popular && <Badge className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-gradient-accent text-primary-foreground">
                  Most Popular
                </Badge>}
              ...
              <CardContent className="space-y-4 flex flex-col flex-grow">
                <ul className="space-y-2">
                  {service.features.map((feature, idx) => <li key={idx} className="flex items-start text-sm text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button variant="outline" className="w-full border-primary/20 text-primary hover:bg-primary/10" onClick={() => setSelectedService(service)}>
                          Learn More
                        </Button>
                      </DialogTrigger>
                      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto bg-background border-border">
                        ...
                        <div className="space-y-6 mt-6">
                          <div>
                            ...
                            <ul className="space-y-2">
                              {service.process.map((step, idx) => <li key={idx} className="flex items-start text-sm text-muted-foreground">
                          ...
                          <div className="pt-4 border-t border-border/20">
                            <div className="flex items-center justify-between">
                              ...
                              <Button className="bg-gradient-accent shadow-accent hover:shadow-luxury transition-all duration-300" onClick={() => {
                    ...
                    <Button className="w-full bg-gradient-accent shadow-accent hover:shadow-luxury transition-all duration-300" onClick={() => {
                  const contactSection = document.getElementById('contact');
                  const serviceTextarea = document.querySelector('textarea[placeholder*="Describe the services"]') as HTMLTextAreaElement;
                  if (contactSection) {
                    contactSection.scrollIntoView({
